...
import { Search, Home, ChevronLeft, ChevronRight, X, Grid, List, SlidersHorizontal, MapPin } from "lucide-react";
import { searchProperties, Property, PropertySearchParams } from "../services/propertyService";
...
export default function SearchResults() {
  ...
  const updateSearchParams = useCallback((newFilters: PropertySearchParams, page = currentPage) => {
  ...
  const handleFilterChange = useCallback((key: keyof PropertySearchParams, value: string | number) => {
  ...
  return (
      <div className="min-h-screen w-full bg-[#f7f5ef]">
        ...
        <section className="container mx-auto px-4 pb-8">
          {loading ? (
              <div className={`grid gap-6 animate-pulse ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              ...
              <>
                <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
                  {properties.map((property) => (
                      ...
                      >
                        <div className={`relative ${viewMode === 'grid' ? 'h-64' : 'w-64 h-48 flex-shrink-0'}`}>
                          <Image
                        ...
                        <div className="p-4 flex-grow">
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="text-xl font-medium line-clamp-2">{property.title}</h3>
                            <span className="text-2xl font-semibold text-green-800 ml-4">
